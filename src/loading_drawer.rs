...
impl LoadingDrawer {
    ...
    fn handle_drawer_tick(&mut self) {
        self.draw_all();
    }
    ...
    pub fn spawn_drawer_engine(mut self) -> JoinHandle<()> {
    ...
    fn init(&mut self) {
        self.set_stdin_mode(self.stdin_enabled); // Trigger the stdin raw mode (or not)
    }
}
...
impl LoadingDrawer {
    ...
    pub fn draw_print_history(&mut self) {
        ...
        let offset: usize = match pos { // Offset height for printing history, which our terminal cursor must jump to, as to avoid overwriting loading bars
            Position::BOTTOM => 0,
            Position::TOP => self.list.len() + 1
        };
        if sz.y <= self.list.len() + 1 { // Accounting for both loading elemenets and splitter line
        ...
        let mut remaining_height: usize = self.get_remaining_height();
    ...
    fn draw_loader(&mut self) {
        let sz: V2Usz = get_terminal_size();
        for (i, elem) in self.list.iter().enumerate() {
            let line = match self.loadingbar_anchor_position {
                Position::TOP => i+1,

...
use crate::{draw_ordering::{DrawOrdering, DrawableElement, DrawableElementFill, DRAW_PRINT_HISTORY, LOADING_BAR}, drawer_helper::{print_splitter_line, set_terminal_pos, LoadingColorScheme, Position}, loading_element::LoadingElement, loading_handler::LoadingHandler, terminal_helper::{get_terminal_size, V2Usz}};
...
impl LoadingDrawer {
    ...
    pub fn draw_all(handler: &mut LoadingHandler) {
        let drawer = handler.drawer;
        drawer.draw_ordering.elements.iter().for_each(|elem| {
            elem.draw(handler);
        });
        self.draw_ordering.fill_element.draw(handler);
    }
    ...
    fn get_remaining_height(&self, handler: &LoadingHandler) -> usize { // WARNING: Make sure that only one drawable uses this function, as only one drawable can take up the "rest" of the space
        return get_terminal_size().y 
            - handler.list.len()
            - (if handler.stdin_enabled {3} else {0});
    }
}
    ...
    pub fn draw_print_history(element: &DrawableElementFill, loading_handler: &mut LoadingHandler) {
        loading_handler.flush_print_buffer();
        let history: &VecDeque<String> = &loading_handler.print_history;
        ...
        let offset: usize = match Position::BOTTOM { // Offset height for printing history, which our terminal cursor must jump to, as to avoid overwriting loading bars
            Position::BOTTOM => 0,
            Position::TOP => loading_handler.list.len() + 1
        };
        if sz.y <= loading_handler.list.len() + 1 { // Accounting for both loading elemenets and splitter line
        ...
        let mut remaining_height: usize = loading_handler.drawer.get_remaining_height(loading_handler);
...
pub fn draw_loader(element: &DrawableElement, loading_handler: &mut LoadingHandler) {
    let sz: V2Usz = get_terminal_size();
    for (i, elem) in loading_handler.list.iter().enumerate() {
        ...
        match &loading_handler.drawer.color_scheme {
